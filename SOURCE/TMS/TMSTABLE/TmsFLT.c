        ...
        while (1)
        {
		    ...
		    szTMSFLTRec[k - 1] == 0x00)
                ...
                if (i > lnTMSFLTLength)
                {
                        if (ginDebug == VS_TRUE)
                        {
                                inDISP_LogPrintf("TMSFLT unpack ERROR.");
                        }
...
int inSaveTMSFLTRec(int inTMSFLTRec)
{
	...
	char szTempTitle[] = "inSaveTMSFLTRec";
	...
	inTIME_UNIT_InitCalculateRunTimeGlobal_Start(&ulRunTime, "inSaveTMSFLTRec INIT" );
#endif

	inTempIndex = inTMSFLTRec;
	
	memset(szTempFIleName, 0x00, sizeof(szTempFIleName));
	strcat( szTempFIleName, _TMSFLT_FILE_NAME_);
	
	memset(szTempBakFileName, 0x00, sizeof(szTempBakFileName));
	strcat( szTempBakFileName, _TMSFLT_FILE_NAME_BAK_);
	...
	usRetVal = CTOS_FileGetSize((unsigned char *)_TMSFLT_FILE_NAME_, &ulFileSize);	
	...
	ulPackCount += strlen(srTMSFLTRec.szTMSFileIndex);
	uszWriteBuff_Record[ulPackCount] = 0x2C;
	ulPackCount++;

	/* TMSFileAttribute */
	memcpy(&uszWriteBuff_Record[ulPackCount], &srTMSFLTRec.szTMSFileAttribute[0], strlen(srTMSFLTRec.szTMSFileAttribute));
	ulPackCount += strlen(srTMSFLTRec.szTMSFileAttribute);
	uszWriteBuff_Record[ulPackCount] = 0x2C;
	ulPackCount++;

	/* TMSFilePathName */
	memcpy(&uszWriteBuff_Record[ulPackCount], &srTMSFLTRec.szTMSFilePathName[0], strlen(srTMSFLTRec.szTMSFilePathName));
	ulPackCount += strlen(srTMSFLTRec.szTMSFilePathName);
	uszWriteBuff_Record[ulPackCount] = 0x2C;
	ulPackCount++;

	/* TMSFileSize */
	memcpy(&uszWriteBuff_Record[ulPackCount], &srTMSFLTRec.szTMSFileSize[0], strlen(srTMSFLTRec.szTMSFileSize));
	ulPackCount += strlen(srTMSFLTRec.szTMSFileSize);
...
#ifdef __COUNT_FUNC_TIME__
	inTIME_UNIT_GetRunTimeGlobal(ulRunTime, "inSaveTMSFLTRec END");
#endif
...
int inGetTMSFileIndex(char* szTMSFileIndex)
{
        ...
        if (szTMSFileIndex == NULL || strlen(srTMSFLTRec.szTMSFileIndex) <= 0 || strlen(srTMSFLTRec.szTMSFileIndex) > 2)
        {
                ...
                if (ginDebug == VS_TRUE)
                {
                        ...
                        inDISP_LogPrintf("inGetTMSFileIndex() ERROR !!");
                        
			if (szTMSFileIndex == NULL)
                        {
                                inDISP_LogPrintf("szTMSFileIndex == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
...
int inSetTMSFileIndex(char* szTMSFileIndex)
{
        ...
        if (szTMSFileIndex == NULL || strlen(szTMSFileIndex) <= 0 || strlen(szTMSFileIndex) > 2)
        {
                ...
                if (ginDebug == VS_TRUE)
                {
                        ...
                        inDISP_LogPrintf("inSetTMSFileIndex() ERROR !!");
                        if (szTMSFileIndex == NULL)
                        {
                                inDISP_LogPrintf("szTMSFileIndex == NULL");
                        }
...
int inGetTMSFileAttribute(char* szTMSFileAttribute)
{
        ...
        if (szTMSFileAttribute == NULL || strlen(srTMSFLTRec.szTMSFileAttribute) <= 0 || strlen(srTMSFLTRec.szTMSFileAttribute) > 1)
        {
                ...
                if (ginDebug == VS_TRUE)
                {
                        ...
                        inDISP_LogPrintf("inGetTMSFileAttribute() ERROR !!");

                        if (szTMSFileAttribute == NULL)
                        {
                                inDISP_LogPrintf("szTMSFileAttribute == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
...
int inSetTMSFileAttribute(char* szTMSFileAttribute)
{
        ...
        if (szTMSFileAttribute == NULL || strlen(szTMSFileAttribute) <= 0 || strlen(szTMSFileAttribute) > 1)
        {
                ...
                if (ginDebug == VS_TRUE)
                {
                        ...
                        inDISP_LogPrintf("inSetTMSFileAttribute() ERROR !!");
...
int inGetTMSFilePathName(char* szTMSFilePathName)
{
        ...
        if (szTMSFilePathName == NULL || strlen(srTMSFLTRec.szTMSFilePathName) <= 0 || strlen(srTMSFLTRec.szTMSFilePathName) > 60)
        {
                ...
                if (ginDebug == VS_TRUE)
                {
                        ...
                        inDISP_LogPrintf("inGetTMSFilePathName() ERROR !!");

                        if (szTMSFilePathName == NULL)
                        {
                                inDISP_LogPrintf("szTMSFilePathName == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
...
int inSetTMSFilePathName(char* szTMSFilePathName)
{
        ...
        if (szTMSFilePathName == NULL || strlen(szTMSFilePathName) <= 0 || strlen(szTMSFilePathName) > 60)
        {
                ...
                if (ginDebug == VS_TRUE)
                {
                        ...
                        inDISP_LogPrintf("inSetTMSFilePathName() ERROR !!");
...
int inGetTMSFileSize(char* szTMSFileSize)
{
        ...
        if (szTMSFileSize == NULL || strlen(srTMSFLTRec.szTMSFileSize) <= 0 || strlen(srTMSFLTRec.szTMSFileSize) > 10)
        {
                ...
                if (ginDebug == VS_TRUE)
                {
                        ...
                        inDISP_LogPrintf("inGetTMSFileSize() ERROR !!");

                        if (szTMSFileSize == NULL)
                        {
                                inDISP_LogPrintf("szTMSFileSize == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
...
int inSetTMSFileSize(char* szTMSFileSize)
{
        ...
        if (szTMSFileSize == NULL || strlen(szTMSFileSize) <= 0 || strlen(szTMSFileSize) > 10)
        {
                ...
                if (ginDebug == VS_TRUE)
                {
                        ...
                        inDISP_LogPrintf("inSetTMSFileSize() ERROR !!");
